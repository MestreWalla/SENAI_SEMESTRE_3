...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        // Atualiza o registro
        $stmt = $pdo->prepare('UPDATE contatos SET nome = ?, email = ?, cell = ?, cadastro = ? WHERE id_contato = ?');
        $stmt->execute([$nome, $email, $cell, $cadastro, $_GET['id']]);
...
<div class="content update">
    <h2><i class="fas fa-edit"></i> Atualizar Pedido <?=$contato['nome']?></h2>
    <form action="update.php?id=<?=$contato['id_contato']?>" method="post">
        <label for="id_contato">ID</label>
        <label for="nome">Nome</label>
        <input type="text" name="id" placeholder="" value="<?=$contato['id_contato']?>" id="id_contato" readonly>
        ...
        <input type="text" name="cell" placeholder="(XX) X.XXXX-XXXX" value="<?=$contato['cell']?>" id="cell">
